...
import { IRetailInsured } from "~/interfaces/retail";

const RetailBeneficiaries = ({ retail }: any) => {
  const { subscriptionItem, getSubscriptionById } = useRetail();

  const [rutInsured, setRutInsured] = useState("");
  const [insured, setInsured] = useState<IRetailInsured | undefined>(undefined);
