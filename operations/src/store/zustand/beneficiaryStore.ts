...
export const beneficiaryStore = create<beneficiaryState>((set, get) => ({
  ...
  upsertBeneficiary: async (data: IBeneficiary) => {
    try {
      ...
      customerStore.setState((customerStore) => ({
        ...customerStore,
        beneficiaryList: customerStore.beneficiaryList.map((beneficiary) =>
  ...
  removeLeadBeneficiary: async (  insured_id: string, beneficiary_id:string, subscription_id:string ) => {
    try {
      ...
      customerStore.setState((customerStore) => ({
        ...customerStore,
        beneficiaryList: customerStore.beneficiaryList.filter((beneficiary) => beneficiary.id !== response?.data.beneficiary_id),
      }));
