...
import { reimbursmentResponse } from "~/interfaces/reimbursment";
...
export const columns: ColumnDef<DataItem>[] = [
  ...
  {
    accessorKey: "casemodel.register_amount",
    header: "Solicitado",
    cell: ({ row }) => {
      const required = row.original.required_amount;
      return required
        ? Number(required).toLocaleString("es-CL", {
            style: "currency",
            currency: "CLP",
          })
        : "0";
