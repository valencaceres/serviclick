...
import {
  ...
  createLead,
  createLeadInsured,
  deleteLeadBeneficiaries,
  deleteLeadInsured,
  createLeadBeneficiary,
  sendPaymentLink,
  updateLeadPaymentType,
} from "../lead";
...
const createSubscriptionGateway = async (
  ...
  try {
    ...
    if (method === "reveniu") {
      createLogger.info({
        url: `${config.apiPaymentURL}/subscription/create?method=reveniu&plan_id=${product.plan_id}`,
        method: "POST",
        body: {
          dataToSend,
        },
      ...
      const subscriptionReveniuResponse = await ApiPaymentInstance.post(
        `/subscription/create?method=${method}&plan_id=${product.plan_id}`,
        dataToSend
      );
      const {
        id: subscription_id,
      ...
      const subscriptionGatewayResponse = await Subscription.getCollector(
        method
      );
    ...
    if (method === "flow") {
      createLogger.info({
        url: `${config.apiPaymentURL}/subscription/create?method=${method}&plan_id=${product.plan_id}`,
        method: "POST",
        body: {
      ...
      const { subscriptionId, planId, paymentLink } =
        subscriptionFlowResponse.data.data.data;
      const leadResponse = await Lead.updateSubscriptionGateway(
        lead_id,
        subscriptionId
      );
...
export { createControllerGateway, getByIdControllerGateway };
