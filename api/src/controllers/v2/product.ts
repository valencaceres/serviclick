...
const createProductPlanGateway = async (
  ...
  try {
    ...
    if (yearlyprice && yearlyprice > 0) {
      ...
      try {
        const planResponse = ApiPaymentInstance.get(
          `/plan/${yearly_plan_id_new}`
        );
      } catch (err) {
        createLogger.error({
      ...
      const planResponseYearly = await ApiPaymentInstance.post(
        "/plan/upsert",
        dataToSend
      );
    ...
    if (companyprice && companyprice > 0) {
      ...
      if (customerprice && customerprice > 0) {
        const planResponseCompany = await ApiPaymentInstance.post(
          "/plan/upsert",
          dataToSend
        );
    ...
    if (customerprice && customerprice > 0) {
      try {
        const planResponse = ApiPaymentInstance.get(`/createProductPlan`);
      } catch (err) {
        createLogger.error({
          controller: "product/createProductPlans",
          error: (err as Error).message,
...
const getPlanByIdAndBrokerId = async (req: any, res: any) => {
  try {
    ...
    const response = await ProductPlan.getPlanByIdAndBrokerId(
      broker_id,
      productplan_id
    );
    if (!response?.success) {
    ...
    console.log(
      "%capisrccontrollersproduct.ts:1745 response",
      "color: #007acc;",
      response
    );
    return res
