...
const assignPassword: any = async (id: string, password: string) => {
  try {
    ...
    const data = await pool.query("UPDATE app.userinsured SET hash = $2 WHERE id = $1 returning hash", [
      id,
      hash,
    ]);
...
const getByEmailWithId: any = async (email: string) => {
  try {
    const result = await pool.query(
      `select
        USR.id as user_id,
    ...
    const data = await formatDataInsuredWithId(result.rows);
...
const formatDataInsuredWithId = async (data: any) => {
  const {
    id,
    user_id,
    rut,
...
export { create, assignPassword, getByEmail, getByEmailWithId };
