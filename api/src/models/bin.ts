...
const createBin = async (broker_id: string, holding: string, brand: string, bin: number, product: string, type: string) => {
  try {
...
const getById = async (bin: number) => {
  try {
    const response = await pool.query('SELECT * FROM app.bin WHERE bin = $1', [bin]);
