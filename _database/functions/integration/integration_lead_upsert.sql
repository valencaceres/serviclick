...
BEGIN
	...
	IF p_holder is not null THEN
        ...
        p_holder_birth_date := p_holder->>'birthDate';
       	...
       	IF p_holder_district_id IS NOT NULL THEN
			...
			INTO	p_district_name
			FROM	app.district
			WHERE	id = p_holder_district_id;
		END IF;
	...
	IF p_beneficiaries IS NOT NULL THEN
        ...
        LOOP
			...
			IF p_beneficiary_district_id IS NOT NULL THEN
        		SELECT	district_name
				INTO	p_beneficiary_district_name
				FROM	app.district
				WHERE	id = p_beneficiary_district_id;
    		END IF;           
           
			SELECT	name
			INTO	p_beneficiary_relationship
            FROM	app.relationship
			WHERE	id = p_beneficiary_relationship_id;
		
			SELECT	ben.beneficiary_id
			FROM	app.beneficiary_upsert(
					p_beneficiary_rut, 
            		p_beneficiary_name, 
                	...
                	p_beneficiary_email, 
                	p_beneficiary_phone, 
                	p_beneficiary_birth_date) AS ben
	...
	IF p_product_id IS NOT NULL THEN
	    ...
	    SELECT	ppl.plan_id AS productplan_id,
	    		COALESCE(MAX(pas.lack), 1),
				MAX(ppl.price) AS price,
				MAX(ppl.frequency) AS frequency_code 
	    INTO	p_plan_id,
	    		p_lack,
				p_price,
				p_frequency
	    FROM	app.productassistance pas
				    INNER JOIN app.productplan ppl ON pas.product_id = ppl.product_id AND ppl.TYPE = 'customer'
