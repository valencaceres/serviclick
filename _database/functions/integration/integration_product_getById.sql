-- DROP FUNCTION app.integration_product_get_by_id(uuid, uuid);

CREATE OR REPLACE FUNCTION app.integration_product_get_by_id(p_retail_id uuid, p_product_id uuid)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	SELECT	JSON_BUILD_OBJECT(
			...
			'values', (
				SELECT 	JSON_AGG(DISTINCT val.name)
	...
	WHERE	ppl.agent_id = p_retail_id AND
			pro.id = p_product_id AND
			ppl.TYPE = 'customer';	
