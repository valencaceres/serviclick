CREATE OR REPLACE FUNCTION app.retail_get_collection_by_id(p_id uuid)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
...
begin
	...
	select	MAX(bro.id::varchar)::uuid as retail_id,
			MAX(bro.rut) as retail_rut,
	...
	from	app.lead lea
			    ...
			    inner join app.retail bro on lea.agent_id = bro.id
